...
public class EvolutionService {
    ...
    public String evolve(Player player, PlayerProfile profile, EconomyService economy) {
        if (profile == null || !profile.hasClass()) return "no-class";

        var next = getNextEvolution(profile);
        if (next == null) return "already-max";

        if (profile.getClassLevel() < next.requiredLevel) return "not-enough-level";

        // стоимость (деньги + предметы)
        if (next.costMoney > 0) {
            boolean ok = economy.withdraw(player, next.costMoney);
            if (!ok) return "no-money";
        }
        if (next.costItems != null && !next.costItems.isEmpty()) {
            if (!hasItems(player, next.costItems)) return "no-items";
        ...
        // если это последняя эволюция — помечаем класс как пройденный до конца (mastered)
        if (getNextEvolution(profile) == null) {
            profile.addMastered(profile.getClassId());
        }

        // награда
        if (next.rewardMoney > 0) economy.deposit(player, next.rewardMoney);
        if (next.rewardItems != null && !next.rewardItems.isEmpty()) giveItems(player, next.rewardItems);
    ...
    private void takeItems(Player player, List<ItemAmount> items) {
        for (ItemAmount ia : items) {
            ...
            for (int slot = 0; slot < inv.getSize(); slot++) {
                ...
                if (stack.getType() != ia.material()) continue;
    ...
    public EvolutionDef getCurrentEvolution(PlayerProfile profile) {
        if (profile == null || !profile.hasClass()) return null;

        var def = classConfig.get(profile.getClassId());
        if (def == null || def.evolutions == null || def.evolutions.isEmpty()) return null;

        String curId = profile.getEvolution();
        if (curId == null || curId.isBlank()) return def.evolutions.get(0);

        for (var e : def.evolutions) {
            if (e.id.equalsIgnoreCase(curId)) return e;
        }
    ...
    public EvolutionDef getNextEvolution(PlayerProfile profile) {
        ...
        if (nextIndex >= def.evolutions.size()) return null;
    ...
    public boolean canChangeClass(Player player, PlayerProfile profile) {
        ...
        if (profile.isStarterClass()) return true;
