    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (sub) {
            ...
            case "admin" -> {
                ...
                if (args.length < 3) {
                    // Добавь эту строку в messages.yml
                    // usage-admin: "&cИспользование: /class admin <player> <classId>"
                    p.sendMessage(messages.get("usage-admin"));
                    return true;
                }
                ...
                if (target == null) {
                    ...
                    p.sendMessage(messages.get("player-not-found"));
                    return true;
                }

                ClassId classId = ClassId.fromString(args[2]);
                if (classId == null) {
                    // unknown-class: "&cНеизвестный класс: {class}"
                    p.sendMessage(messages.format("unknown-class", Map.of(
                            "class", args[2])));
                    return true;
                }
                ...
                // admin-set: "&aВы назначили игроку {player} класс {class}."
                p.sendMessage(messages.format("admin-set", Map.of(
                        "player", target.getName(),
                        "class", classId.name())));

                // admin-set-self: "&aАдмин назначил вам класс {class}."
                target.sendMessage(messages.format("admin-set-self", Map.of(
                        "class", classId.name())));
                return true;
            }
